...
const modules = await ihkContentService.getModulesByThreeTierCategory('daten-prozessanalyse');
...
const aeModules = await ihkContentService.getModulesByThreeTierCategory('anwendungsentwicklung');
const dpaModules = await ihkContentService.getModulesByThreeTierCategory('daten-prozessanalyse');
const generalModules = await ihkContentService.getModulesByThreeTierCategory('allgemein');
```
...
async function organizeContentBySpecialization(specializationId) {
  const organizedContent = await ihkContentService.getContentWithCategoryInfo(specializationId);
...
const mappingResult = categoryMappingService.mapToThreeTierCategory(contentItem);
...
async function compareResults(contentId) {
  // Legacy approach
  const legacyModules = await ihkContentService.getModulesByCategory('BP-DPA-01');
  
  // New approach
  const newModules = await ihkContentService.getModulesByThreeTierCategory('daten-prozessanalyse');
...
function filterModulesByType(modules, type) {
  return modules.filter(module => {
    switch (type) {
      ...
      case 'ae':
        return module.category.startsWith('BP-') && !module.category.includes('DPA');
      case 'general':
...
function filterModulesByType(modules, type) {
  ...
  return modules.filter(module => 
    module.threeTierCategory === targetCategory
  );
}
```
...
async function getRelevantContent(specializationId) {
  ...
  Object.entries(organizedContent).forEach(([categoryId, categoryData]) => {
    if (categoryData.relevance === 'high' || categoryData.relevance === 'medium') {
...
async function searchWithContext(query, userSpecialization) {
  ...
  const relevantResults = allResults.filter(item => {
    const relevance = specializationService.getCategoryRelevance(
      item.category, 
      userSpecialization
    );
    ...
    const aRelevance = specializationService.getCategoryRelevance(a.category, userSpecialization);
    const bRelevance = specializationService.getCategoryRelevance(b.category, userSpecialization);
...
async function searchWithContext(query, userSpecialization) {
  const searchResults = await ihkContentService.searchThreeTierCategories(query, {
...
function getCategoryInfo(contentItem) {
  try {
    ...
    if (legacyCategory) {
      if (legacyCategory.includes('BP-DPA')) return { category: 'daten-prozessanalyse', source: 'legacy' };
      if (legacyCategory.includes('BP-AE') || legacyCategory.startsWith('BP-')) return { category: 'anwendungsentwicklung', source: 'legacy' };
    ...
    console.warn('Error determining category for content:', contentItem.id, error);
...
async function validateMigration() {
  ...
  try {
    ...
    Object.entries(stats.categories).forEach(([categoryId, categoryStats]) => {
      const percentage = (categoryStats.contentCounts.total / totalContent) * 100;
      if (percentage < 5) {
        issues.push(`Category ${categoryId} has very low content (${percentage.toFixed(1)}%)`);
      }
    });
    ...
    if (categoryMappingService) {
      ...
      if (validation.errors.length > 0) {
        issues.push(`${validation.errors.length} category mapping errors found`);
      }
    }
...
describe('Three-Tier Category Migration', () => {
  ...
  test('should preserve backward compatibility', async () => {
    const legacyResult = await ihkContentService.getModulesByLegacyCategory('BP-DPA-01');
    const newResult = await ihkContentService.getModulesByThreeTierCategory('daten-prozessanalyse');
...
describe('Migration Integration', () => {
  test('should maintain user progress across migration', async () => {
    ...
    const userProgress = {
      ...
      quizAttempts: [
        { quizId: 'bp-dpa-01-quiz', score: 85 }
      ]
    };
