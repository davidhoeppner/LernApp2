...
- You WILL ground findings in repository documentation (`README.md`, `requirements.md`, `design.md`, `modules.md`, `RESPONSIVE_TEST_GUIDE.md`, `VALIDATION_GUIDE.md`) and match the architecture implemented in `src/`, `scripts/`, `public/`, and `data/`
- You WILL map each discovery to the files, services, components, datasets, and npm scripts it affects (lint, test, build, validate, preview)
...
- Root documentation (`README.md`, `requirements.md`, `design.md`, `modules.md`, `RESPONSIVE_TEST_GUIDE.md`, `VALIDATION_GUIDE.md`, `ACCESSIBILITY_FEATURES.md`, `PERFORMANCE_REPORT.md`)
- Source directories (`src/`, `scripts/`, `data/`, `public/`) to capture components, services, utilities, JSON schemas, and build automation
- Tooling and configuration files (`package.json`, `vite.config.js`, `eslint.config.js`, `jsconfig.json`, `netlify.toml`, `vercel.json`)
- `.github/instructions/` for process guidance and quality gates
...
{{data_contracts_and_validation_notes}}

### Automation & QA Signals

- npm run {{script_name_1}} → {{script_result_1}}
- npm run {{script_name_2}} → {{script_result_2}}
- Manual QA → {{manual_validation_focus}}
...
- Using `#codebase` to analyze project files, component/service patterns, and module boundaries
- Using `#search` to find specific implementations, configurations, lint rules, and routing patterns
- Using `#usages` to understand how utilities and services are consumed across the app
- Executing read operations to review entire files for conventions, data contracts, and accessibility considerations
- Referencing `.github/instructions/` plus the documentation listed in the Research Standards section for established guidelines

You WILL conduct comprehensive external research by:

- Using `#fetch` to gather official documentation (for example Vite, Vitest, ESLint, Prettier, marked, highlight.js, WAI-ARIA, MDN)
- Using `#githubRepo` to research implementation patterns from high-quality Vanilla JS or Vite-based projects
- Reviewing npm package documentation for dependencies declared in `package.json`
- Consulting W3C accessibility guidance and modern frontend best practices to validate UI/UX recommendations
...
- "Research updated Vite build optimization techniques"
- "Investigate Vitest patterns for complex service/state testing"
- "Document best practices for marked.js and highlight.js integration"

You WILL perform project analysis research including:

- "Analyze our service and component architecture under src/"
- "Review how we manage JSON content, validators, and migrations"
- "Summarize how automation scripts under scripts/ enforce quality gates"

You WILL execute comparative research including:

- "Compare alternative routing strategies compatible with our Router service"
- "Evaluate approaches for persistence across StorageService and StateManager"
- "Assess accessibility patterns for complex interactive components (e.g., WheelView, quizzes)"
