...
class PerformanceOptimizationService {
  constructor(ihkContentService, categoryMappingService, advancedCachingService) {
  ...
  _buildFilterIndexes(allContent) {
    ...
    allContent.forEach(item => {
      ...
      if (item.specializationRelevance) {
        Object.entries(item.specializationRelevance).forEach(([spec, relevance]) => {
  ...
  async getContentByThreeTierCategory(categoryId, options = {}) {
    ...
    try {
      ...
      if (this.advancedCachingService) {
        try {
          const cachedResult = await this.advancedCachingService.getContentByCategory(categoryId, options);
          if (cachedResult) {
            this.performanceMetrics.cacheHits++;
            const duration = performance.now() - startTime;
            this._recordFilterOperation(categoryId, options, duration, cachedResult.length);
            return cachedResult;
          }
        } catch (cacheError) {
          console.warn('Advanced caching failed, falling back to index-based filtering:', cacheError);
        }
      }
      ...
      const duration = performance.now() - startTime;
      this._recordFilterOperation(categoryId, options, duration, finalResults.length);
  ...
  async searchInCategory(query, categoryId, options = {}) {
    ...
    try {
      ...
      if (this.advancedCachingService) {
        try {
          const cachedResult = await this.advancedCachingService.searchContent(query, categoryId, options);
          if (cachedResult) {
            this.performanceMetrics.cacheHits++;
            const duration = performance.now() - startTime;
            this._recordSearchOperation(query, categoryId, options, duration, cachedResult.length);
            return cachedResult;
          }
        } catch (cacheError) {
          console.warn('Advanced search caching failed, falling back to index-based search:', cacheError);
        }
      }
      ...
      const categoryMatches = this._intersectSets(matchingIds, categoryContentIds);
      ...
      // Record performance metrics
      const duration = performance.now() - startTime;
      this._recordSearchOperation(query, categoryId, options, duration, finalResults.length);
  ...
  _calculateSearchScore(content, searchTerms) {
    ...
    searchTerms.forEach(term => {
      ...
      if (content.tags && content.tags.some(tag => tag.toLowerCase().includes(term))) {
        score += 5;
      }
      ...
      if (searchableText.includes(term + ' ') || searchableText.includes(' ' + term)) {
        score += 2;
      }
    });
  ...
  async _ensureIndexesReady() {
    ...
    if (indexAge > 300000 || this.categoryIndexes.size === 0) { // 5 minutes
  ...
  async _fallbackCategorySearch(query, categoryId, options) {
    try {
      const categoryContent = await this._fallbackCategoryFilter(categoryId, options);
  ...
  getPerformanceMetrics() {
    ...
    const targetMeetRate = recentOperations.length > 0
      ? recentOperations.filter(op => op.meetsTarget).length / recentOperations.length * 100
      : 100;
    ...
    return {
      ...
      summary: {
        ...
        cacheHitRate: this.performanceMetrics.cacheHits + this.performanceMetrics.cacheMisses > 0
          ? Math.round(this.performanceMetrics.cacheHits / 
