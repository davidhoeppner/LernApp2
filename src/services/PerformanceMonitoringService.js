...
class PerformanceMonitoringService {
  ...
  _checkPerformanceThresholds(operationType, metric) {
    ...
    if (metric.memoryUsage > this.metrics.thresholds.memoryUsage) {
      this._createAlert('memory', {
        ...
        severity: metric.memoryUsage > this.metrics.thresholds.memoryUsage * 1.5 ? 'high' : 'medium',
        timestamp: metric.timestamp
      });
    }
  }
  ...
  _collectRealTimeMetrics() {
    ...
    for (const [operationType, metrics] of this.metrics.operations.entries()) {
      ...
      if (recentMetrics.length > 0) {
        realtimeData.operations[operationType] = {
          ...
          avgResultCount: recentMetrics.reduce((sum, m) => sum + (m.resultCount || 0), 0) / recentMetrics.length
        };
      }
    }
    ...
    if (this.dashboardData.realTimeMetrics.length > 60) { // Last hour
      this.dashboardData.realTimeMetrics = this.dashboardData.realTimeMetrics.slice(-60);
    }
  }
  ...
  _calculateAggregatedStats() {
    ...
    for (const [windowName, windowSize] of Object.entries(timeWindows)) {
      ...
      for (const [operationType, metrics] of this.metrics.operations.entries()) {
        const windowMetrics = metrics.filter(m => now - m.timestamp <= windowSize);
        
        if (windowMetrics.length > 0) {
          this.dashboardData.aggregatedStats[windowName][operationType] = {
            totalOperations: windowMetrics.length,
            avgDuration: windowMetrics.reduce((sum, m) => sum + m.duration, 0) / windowMetrics.length,
            p95Duration: this._calculatePercentile(windowMetrics.map(m => m.duration), 95),
            p99Duration: this._calculatePercentile(windowMetrics.map(m => m.duration), 99),
            cacheHitRate: windowMetrics.filter(m => m.cacheHit).length / windowMetrics.length * 100,
            errorRate: windowMetrics.filter(m => m.error).length / windowMetrics.length * 100,
            throughput: windowMetrics.length / (windowSize / 1000), // operations per second
            avgMemoryUsage: windowMetrics.reduce((sum, m) => sum + (m.memoryUsage || 0), 0) / windowMetrics.length
          };
        }
      }
    }
  ...
  _analyzeTrends() {
    ...
    for (const [operationType, metrics] of this.metrics.operations.entries()) {
      const trendMetrics = metrics.filter(m => now - m.timestamp <= trendWindow);
      
      if (trendMetrics.length >= 10) { // Need sufficient data points
        ...
        this.dashboardData.performanceTrends[operationType] = {
          ...
          recommendation: this._generateTrendRecommendation(operationType, trend)
        };
      }
    }
  ...
  _calculateSystemHealth() {
    ...
    const warningAlerts = recentAlerts.filter(alert => alert.severity === 'medium');
    ...
    if (avgPerformance > 200) { // Average operation > 200ms
      return 'warning';
    }
  ...
  _cleanupOldMetrics() {
    ...
    this.metrics.alerts = this.metrics.alerts.filter(
      alert => Date.now() - alert.timestamp <= this.metrics.monitoring.retentionPeriod
    );
    
    // Clean up real-time metrics
    this.dashboardData.realTimeMetrics = this.dashboardData.realTimeMetrics.filter(
      data => Date.now() - data.timestamp <= this.metrics.monitoring.retentionPeriod
    );
  }
  ...
  getPerformanceReport(options = {}) {
    ...
    const report = {
      ...
      summary: {},
      operationTypes: {},
      alerts: this.metrics.alerts.filter(alert => now - alert.timestamp <= timeWindow),
  ...
  _generateRecommendations(report) {
    ...
    for (const [opType, data] of Object.entries(report.operationTypes)) {
      if (data.p95Duration > this.metrics.thresholds[opType] * 1.5) {
        recommendations.push({
          ...
          suggestion: 'Consider optimizing indexes or implementing additional caching'
        });
      }
      ...
      if (data.cacheHitRate < this.metrics.thresholds.cacheHitRate) {
        recommendations.push({
          ...
          suggestion: 'Review caching strategy and consider preloading frequently accessed data'
        });
      }
    }
    ...
    if (report.alerts.some(alert => alert.type === 'memory')) {
      recommendations.push({
        ...
        suggestion: 'Review cache sizes and implement more aggressive cleanup policies'
      });
    }
    
    // Check for trending issues
    for (const [opType, trend] of Object.entries(this.dashboardData.performanceTrends)) {
      if (trend.direction === 'increasing' && trend.confidence === 'high') {
        recommendations.push({
          type: 'trend',
          priority: 'medium',
