...
console.log(`✅ Found ${correctPatternsFound}/${newPatterns.length} correct patterns`);

// Check that renderQuizGrid is not called in refresh
const renderQuizGridInRefresh = /const newQuizGrid = this\.renderQuizGrid\(\)/.test(fileContent);
if (renderQuizGridInRefresh) {
  console.log('❌ Still calling renderQuizGrid() in refresh (creates nested grid)');
} else {
  console.log('✅ No longer calling renderQuizGrid() in refresh');
}

// Check for direct content manipulation
const directContentManipulation = fileContent.includes('quizGridContainer.appendChild(card)');
...
// Final result
console.log('\n' + '='.repeat(50));
if (!hasOldPattern && correctPatternsFound >= 3 && !renderQuizGridInRefresh && directContentManipulation) {
  console.log('🎉 SUCCESS: Nested grid fix applied correctly!');
  console.log('\n📋 Fix Summary:');
  console.log('✅ Removed nested quiz grid creation');
  console.log('✅ Quiz cards are now added directly to existing grid container');
  ...
  console.log(`   - Correct patterns found: ${correctPatternsFound}/${newPatterns.length}`);
  console.log(`   - renderQuizGrid removed from refresh: ${!renderQuizGridInRefresh ? 'Yes' : 'No'}`);
  console.log(`   - Direct content manipulation: ${directContentManipulation ? 'Yes' : 'No'}`);
  process.exit(1);
}