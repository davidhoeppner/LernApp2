...
async function testFilterStateManagement() {
  ...
  try {
    ...
    console.log(`  Initial category filter: ${quizListView.currentCategoryFilter}`);
    console.log(`  Initial status filter: ${quizListView.currentStatusFilter}`);
    
    if (quizListView.currentCategoryFilter === 'all' && quizListView.currentStatusFilter === 'all') {
    ...
    const allCategoriesBtn = Array.from(categoryButtons).find(btn => btn.dataset.category === 'all');
    if (allCategoriesBtn) {
      ...
      if (isActive && ariaPressed === 'true') {
        console.log('✅ SUCCESS: "All Categories" button is properly initialized');
      } else {
    ...
    const allQuizzesBtn = Array.from(statusButtons).find(btn => btn.dataset.status === 'all');
    if (allQuizzesBtn) {
