...
async function testEmptyStateAndFeedback() {
  ...
  try {
    ...
    const countDisplayAll = quizListView._createQuizCountDisplay(totalQuizzes, totalQuizzes);
    console.log(`📊 Count display (all): ${countDisplayAll}`);
    
    const countDisplayFiltered = quizListView._createQuizCountDisplay(filteredQuizzes.length, totalQuizzes);
    ...
    quizListView._announceFilterChange('category', 'daten-prozessanalyse', filteredQuizzes.length);
