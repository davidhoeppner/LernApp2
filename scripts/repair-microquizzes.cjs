...
function needsRepair(q) {
  ...
  for (const qq of q.questions) {
    ...
    if (
      qq.type === 'single-choice' &&
      (!qq.options || !qq.options.includes(qq.correctAnswer))
    )
      return true;
    if (qq.type === 'multiple-choice' && !Array.isArray(qq.correctAnswer))
      return true;
    if (qq.type === 'true-false' && typeof qq.correctAnswer !== 'boolean')
      return true;
  }
  return false;
}
...
function repairQuiz(q, file) {
  let changed = false;
  q.description =
    q.description || `Automatisch generiertes Microquiz (${q.id}).`;
  q.category =
    q.category ||
    (q.moduleId && q.moduleId.startsWith('fue-') ? 'FÜ-01' : 'BP-01');
  ...
  while (q.questions.length < 3) {
    ...
    q.questions.push({
      ...
      options:
        idx === 2 ? undefined : idx === 3 ? ['A', 'B', 'C'] : ['A', 'B', 'C'],
  ...
  q.questions = q.questions.slice(0, 3).map((qq, i) => {
    ...
    if (!qq.type) {
      qq.type =
        i === 0 ? 'single-choice' : i === 1 ? 'true-false' : 'multiple-choice';
      changed = true;
    }
    if (!qq.points) {
      qq.points = 1;
    ...
    if (qq.type === 'single-choice') {
      if (!qq.options || qq.options.length < 2) {
        qq.options = ['Option 1', 'Option 2', 'Option 3'];
        changed = true;
      }
      if (!qq.correctAnswer || !qq.options.includes(qq.correctAnswer)) {
        qq.correctAnswer = qq.options[0];
        changed = true;
      }
      ...
      if (typeof qq.correctAnswer !== 'boolean') {
        qq.correctAnswer = true;
        changed = true;
      }
    }
    if (!qq.explanation) {
      qq.explanation = 'Automatisch ergänzt.';
      changed = true;
    }
    return qq;
  });
