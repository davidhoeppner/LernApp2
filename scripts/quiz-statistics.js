...
async function generateQuizStatistics() {
  console.log(
    `${colors.bold}${colors.cyan}=== Quiz Statistics Report ===${colors.reset}\n`
  );
  ...
  for (const file of files) {
    ...
    try {
      ...
      // Difficulty
      if (quiz.difficulty) {
        statistics.difficulties[quiz.difficulty] =
          (statistics.difficulties[quiz.difficulty] || 0) + 1;
      } else {
      ...
      if (quiz.examRelevance) {
        statistics.examRelevance[quiz.examRelevance] =
          (statistics.examRelevance[quiz.examRelevance] || 0) + 1;
      } else {
      ...
      // Category
      if (quiz.category) {
        statistics.categories[quiz.category] =
          (statistics.categories[quiz.category] || 0) + 1;
      }
      ...
      console.log(
        `${colors.red}✗${colors.reset} Error parsing ${file}: ${error.message}`
      );
    }
  }
  ...
  const medianQuestions = statistics.questionCounts.sort((a, b) => a - b)[
    Math.floor(statistics.questionCounts.length / 2)
  ];

  // Print overview
  console.log(`${colors.bold}${colors.cyan}=== Overview ===${colors.reset}`);
  console.log(
    `Total Quizzes: ${colors.bold}${statistics.totalQuizzes}${colors.reset}`
  );
  console.log(
    `Total Questions: ${colors.bold}${statistics.totalQuestions}${colors.reset}`
  );
  console.log(
    `Average Questions per Quiz: ${colors.bold}${avgQuestions.toFixed(1)}${colors.reset}`
  );
  ...
  console.log(
    `Median Questions: ${colors.bold}${medianQuestions}${colors.reset}`
  );
  ...
  console.log(
    `${colors.green}Beginner:${colors.reset} ${statistics.difficulties.beginner} (${((statistics.difficulties.beginner / statistics.totalQuizzes) * 100).toFixed(1)}%)`
  );
  console.log(
    `${colors.yellow}Intermediate:${colors.reset} ${statistics.difficulties.intermediate} (${((statistics.difficulties.intermediate / statistics.totalQuizzes) * 100).toFixed(1)}%)`
  );
  console.log(
    `${colors.red}Advanced:${colors.reset} ${statistics.difficulties.advanced} (${((statistics.difficulties.advanced / statistics.totalQuizzes) * 100).toFixed(1)}%)`
  );
  if (statistics.difficulties.unknown > 0) {
    console.log(
      `${colors.magenta}Unknown:${colors.reset} ${statistics.difficulties.unknown}`
    );
  }
  ...
  console.log(
    `${colors.red}High:${colors.reset} ${statistics.examRelevance.high} (${((statistics.examRelevance.high / statistics.totalQuizzes) * 100).toFixed(1)}%)`
  );
  console.log(
    `${colors.yellow}Medium:${colors.reset} ${statistics.examRelevance.medium} (${((statistics.examRelevance.medium / statistics.totalQuizzes) * 100).toFixed(1)}%)`
  );
  console.log(
    `${colors.green}Low:${colors.reset} ${statistics.examRelevance.low} (${((statistics.examRelevance.low / statistics.totalQuizzes) * 100).toFixed(1)}%)`
  );
  if (statistics.examRelevance.unknown > 0) {
    console.log(
      `${colors.magenta}Unknown:${colors.reset} ${statistics.examRelevance.unknown}`
    );
  }
  ...
  Object.entries(statistics.categories)
    ...
    .forEach(([category, count]) => {
      console.log(
        `${category}: ${count} (${((count / statistics.totalQuizzes) * 100).toFixed(1)}%)`
      );
    });
  ...
  console.log(
    `Single-Choice: ${statistics.questionTypes['single-choice']} (${((statistics.questionTypes['single-choice'] / statistics.totalQuestions) * 100).toFixed(1)}%)`
  );
  console.log(
    `Multiple-Choice: ${statistics.questionTypes['multiple-choice']} (${((statistics.questionTypes['multiple-choice'] / statistics.totalQuestions) * 100).toFixed(1)}%)`
  );
  console.log(
    `True-False: ${statistics.questionTypes['true-false']} (${((statistics.questionTypes['true-false'] / statistics.totalQuestions) * 100).toFixed(1)}%)`
  );
  console.log(
    `Code: ${statistics.questionTypes['code']} (${((statistics.questionTypes['code'] / statistics.totalQuestions) * 100).toFixed(1)}%)`
  );
  ...
  console.log(
    `Quizzes marked as new in 2025: ${colors.bold}${statistics.newIn2025}${colors.reset}`
  );

  // Print quizzes needing attention (< 15 or > 20 questions)
  const needsAttention = statistics.quizzes.filter(
    q => q.questionCount < 15 || q.questionCount > 20
  );
  ...
  if (needsAttention.length > 0) {
    ...
    console.log(
      `${needsAttention.length} quiz(zes) have less than 15 or more than 20 questions:\n`
    );
    ...
    needsAttention
      ...
      .forEach(quiz => {
        ...
        console.log(
          `${color}${quiz.questionCount} questions${colors.reset} - ${quiz.title} (${quiz.file})`
        );
      });
  }
  ...
  statistics.quizzes
    ...
    .forEach(quiz => {
      ...
      const newBadge = quiz.newIn2025
        ? `${colors.cyan}[NEW 2025]${colors.reset} `
        : '';

      console.log(
        `${questionColor}${quiz.questionCount.toString().padStart(2)}${colors.reset} questions | ${newBadge}${quiz.title}`
      );
      console.log(
        `   ${colors.blue}${quiz.category}${colors.reset} | ${quiz.difficulty} | ${quiz.examRelevance} relevance | ${quiz.file}`
      );
    });
