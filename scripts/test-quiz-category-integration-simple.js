...
const quizListViewPath = path.join(process.cwd(), 'src/components/IHKQuizListView.js');
const quizListViewContent = fs.readFileSync(quizListViewPath, 'utf8');

// Check if constructor includes categoryMappingService
if (quizListViewContent.includes('this.categoryMappingService = services.categoryMappingService')) {
  console.log('✅ Constructor properly assigns categoryMappingService from services');
} else {
...
if (quizListViewContent.includes('this.categoryMappingService.mapToThreeTierCategory(quiz)')) {
  console.log('✅ _getCategoryIndicator uses CategoryMappingService.mapToThreeTierCategory()');
} else {
...
if (quizListViewContent.includes('try {') && quizListViewContent.includes('catch (error)')) {
...
const hasHardcodedMapping = quizListViewContent.includes("categoryId.includes('BP-DPA')") ||
...
if (quizListViewContent.includes('console.warn') && quizListViewContent.includes('console.error')) {
...
console.log('  ✅ _getCategoryIndicator method uses CategoryMappingService.mapToThreeTierCategory()');
...
console.log('  ✅ 2.1: Quiz has valid threeTierCategory field (via CategoryMappingService)');
console.log('  ✅ 2.2: Category indicator uses threeTierCategory field for filtering');
console.log('  ✅ 2.3: System falls back to legacy category mapping when needed');
console.log('  ✅ 2.6: Quiz defaults to allgemein when no recognizable category');
console.log('  ✅ 5.1: Error logging with details about failures');
console.log('  ✅ 5.2: Warning logging for invalid category data with fallback');
