...
class MockIHKContentService {
  constructor() {
    ...
    for (let i = 1; i <= 50; i++) {
      this.quizzes.set(`quiz-${i}`, {
        ...
        questions: Array(10)
          .fill()
          .map((_, j) => ({ id: j, question: `Question ${j}` })),
      });
    }
  }
  ...
  async searchInCategory(query, categoryId, options = {}) {
    const categoryContent = await this.getContentByThreeTierCategory(
      categoryId,
      options
    );
...
async function testPerformanceServices() {
  ...
  try {
    ...
    const dpaContent = await perfOptService.getContentByThreeTierCategory(
      'daten-prozessanalyse'
    );
    const filterTime = performance.now() - startTime;

    console.log(
      `   - Category filtering: ${dpaContent.length} items in ${filterTime.toFixed(2)}ms`
    );
    console.log(
      `   - Performance target (<100ms): ${filterTime < 100 ? '✅ PASS' : '❌ FAIL'}`
    );

    // Test search functionality
    const searchStartTime = performance.now();
    const searchResults = await perfOptService.searchInCategory(
      'test',
      'daten-prozessanalyse'
    );
    ...
    console.log(
      `   - Category search: ${searchResults.length} results in ${searchTime.toFixed(2)}ms`
    );
    console.log(
      `   - Search performance (<150ms): ${searchTime < 150 ? '✅ PASS' : '❌ FAIL'}`
    );
    ...
    const cachedContent1 = await cachingService.getContentByCategory(
      'anwendungsentwicklung'
    );
    const cacheMissTime = performance.now() - cacheMissStart;

    const cacheHitStart = performance.now();
    const cachedContent2 = await cachingService.getContentByCategory(
      'anwendungsentwicklung'
    );
    const cacheHitTime = performance.now() - cacheHitStart;

    console.log(
      `   - Cache miss: ${cachedContent1.length} items in ${cacheMissTime.toFixed(2)}ms`
    );
    console.log(
      `   - Cache hit: ${cachedContent2.length} items in ${cacheHitTime.toFixed(2)}ms`
    );
    console.log(
      `   - Cache speedup: ${(cacheMissTime / cacheHitTime).toFixed(1)}x faster`
    );
    ...
    console.log(
      `   - Real-time metrics: ${dashboardData.realTimeMetrics.length} data points`
    );
    ...
    console.log(
      `   - Average duration: ${performanceReport.summary.avgDuration.toFixed(2)}ms`
    );
    ...
    console.log(
      `   - Integration ready: ${integrationService.isReady() ? '✅ YES' : '❌ NO'}`
    );

    // Test integrated operations
    const integratedStart = performance.now();
    const integratedContent =
      await integrationService.getContentByCategory('allgemein');
    const integratedTime = performance.now() - integratedStart;

    console.log(
      `   - Integrated filtering: ${integratedContent.length} items in ${integratedTime.toFixed(2)}ms`
    );

    const integratedSearchStart = performance.now();
    const integratedSearchResults = await integrationService.searchInCategory(
      'module',
      'allgemein'
    );
    ...
    console.log(
      `   - Integrated search: ${integratedSearchResults.length} results in ${integratedSearchTime.toFixed(2)}ms`
    );

    // Get comprehensive report
    const comprehensiveReport = integrationService.getPerformanceReport();
    console.log(
      `   - Health score: ${comprehensiveReport.healthScore.score}/100 (${comprehensiveReport.healthScore.status})`
    );
    console.log(
      `   - Services active: ${Object.values(comprehensiveReport.services).filter(Boolean).length}/4`
    );
    ...
    const totalItems = loadTestResults.reduce(
    ...
    console.log(
      `   - Load test: ${loadTestPromises.length} requests in ${loadTestTime.toFixed(2)}ms`
    );
    console.log(`   - Average per request: ${avgTimePerRequest.toFixed(2)}ms`);
    console.log(`   - Total items retrieved: ${totalItems}`);
    console.log(
      `   - Throughput: ${(loadTestPromises.length / (loadTestTime / 1000)).toFixed(1)} requests/second`
    );
    ...
    console.log(
      `   - Overall health: ${finalReport.healthScore.status} (${finalReport.healthScore.score}/100)`
    );
    console.log(
      `   - Recommendation: ${finalReport.healthScore.recommendation}`
    );

    if (finalReport.optimization?.summary) {
      console.log(
        `   - Cache hit rate: ${finalReport.optimization.summary.cacheHitRate}%`
      );
      console.log(
        `   - Average response time: ${finalReport.optimization.summary.avgDurationMs}ms`
      );
    }
