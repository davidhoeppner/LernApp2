...
async function runComprehensiveIntegrationTests() {
  ...
  try {
    ...
    console.error('❌ Critical error during comprehensive test execution:', error);
  }
  
  const overallEndTime = performance.now();
  const totalExecutionTime = ((overallEndTime - overallStartTime) / 1000).toFixed(2);
  
  // Generate unified report
  generateUnifiedReport(coreTestsReport, migrationTestsReport, contentLoadingTestsReport, totalExecutionTime);
 ...
 */
function generateUnifiedReport(coreReport, migrationReport, contentLoadingReport, executionTime) {
  console.log('\n' + '='.repeat(90));
  console.log('📋 COMPREHENSIVE THREE-TIER CATEGORY SYSTEM TEST REPORT');
  console.log('='.repeat(90));
  
  // Calculate overall statistics
  const totalTests = (coreReport?.totalTests || 0) + (migrationReport?.totalTests || 0) + (contentLoadingReport?.totalTests || 0);
  const totalPassed = (coreReport?.passed || 0) + (migrationReport?.passed || 0) + (contentLoadingReport?.passed || 0);
  const totalFailed = (coreReport?.failed || 0) + (migrationReport?.failed || 0) + (contentLoadingReport?.failed || 0);
  ...
  const overallSuccess = coreSuccess && migrationSuccess && contentLoadingSuccess;
  ...
  console.log(`   Migration & Validation: ${migrationSuccess ? '✅ PASS' : '❌ FAIL'}`);
  console.log(`   Overall System: ${overallSuccess ? '✅ READY' : '❌ NOT READY'}`);
